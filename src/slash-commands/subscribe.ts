...
export const run: SlashCommandRunFunction = async (interaction) => {
    ...
    return void interaction.reply({
      embeds: [
        new EmbedBuilder()
          .setColor(process.env.EMBED_COLOR)
          .setDescription(
            `Hey **${interaction.user.username}**, this email is not subscribed yet. You can purchase a new subscription at ${process.env.STRIPE_PAYMENT_LINK} and run this command again.`
          ),
      ],
      ephemeral: true,
    });
  ...
  const existingEmailCustomer = await Postgres.getRepository(
    DiscordCustomer
  ).findOne({
    where: {
      email,
  ...
  const member = await interaction.guild?.members
    ...
    ?.catch(() => {});
  await (member as GuildMember)?.roles
    .add(process.env.PAYING_ROLE_ID)
    ?.catch(() => {});

  (
    member?.guild.channels.cache.get(process.env.LOGS_CHANNEL_ID) as TextChannel
  ).send(
    `:arrow_upper_right: **${member?.user?.tag || "Unknown#0000"}** (${
      customer.discordUserId
    }, <@${customer.discordUserId}>) has been linked to \`${customer.email}\`.`
  );

  return void interaction.reply({
    ephemeral: true,
    embeds: successEmbed(
      `Welcome, you are eligible to the exclusive Discord access!`
    ).embeds,
  });
};
