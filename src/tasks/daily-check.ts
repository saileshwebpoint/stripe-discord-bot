...
const getExpiredEmbed = (daysLeft: 0 | 1 | 2): EmbedBuilder => {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      } to keep your exclusive access! ${
        daysLeft > 0
          ? `Your subscription expires within ${daysLeft * 24} hours.`
          : ""
      }`
    );
  return embed;
 ...
 * 4) Send logs
 */
const makeMemberExpire = async (
  customer: DiscordCustomer,
  member: GuildMember,
  guild: Guild
) => {
...
export const run = async () => {
  ...
  for (const customer of customers) {
    ...
    if (!customerId) {
      console.log(
        `[Daily Check] Could not find customer id for ${customer.email}`
      );
      continue;
    }
    ...
    if (activeSubscriptions.length > 0 || hasLifetime) {
      console.log(
        `${customer.email} has active subscriptions or a lifetime one.`
      );

      if (
        !customer.hadActiveSubscription ||
        customer.firstReminderSentDayCount
      ) {
        await Postgres.getRepository(DiscordCustomer).update(customer.id, {
